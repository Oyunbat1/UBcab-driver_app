...
class TripController extends GetxController {
  ...
  AudioService get _audio => Get.isRegistered<AudioService>() ? Get.find<AudioService>() : Get.put(AudioService(), permanent: true);
  ...
  void _startListeningForTrips() {
    ...
    _requestsSubscription = tripApi.watchRequestedTrips().listen((snapshot) {
  ...
  Future<void> acceptIncomingTrip() async {
    ...
    state.activeTrip.value = {...tripData, 'driverId': driverId, 'status': 'accepted'};
    ...
    _activeTripSubscription = tripApi.watchTrip(tripId).listen((snapshot) {
      // Trip doc устсан (rider устгасан эсвэл гараар устгасан) бол aylal
      // duusсан гэж үзээд home ruu butsana - эс бол дэлгэц гацна.
      if (!snapshot.exists) {
        Get.snackbar('Аялал', 'Аялал олдсонгүй');
        _resetActiveTrip();
      ...
      if (status == 'cancelled') {
        Get.snackbar('Аялал', 'Зорчигч аяллыг цуцаллаа');
        _resetActiveTrip();
        Get.offAllNamed(AppRoutes.home);
      }
    });
  ...
  Future<void> _fetchRiderInfo(String riderId) async {
    try {
      ...
      if (data == null) return;
      state.riderName.value = (data['name'] as String?)?.trim().isNotEmpty == true ? data['name'] : 'Зорчигч';
  ...
  Future<void> completeTrip() async {
    ...
    final driverId = trip['driverId'] as String? ?? FirebaseAuth.instance.currentUser?.uid ?? '';

    _audio.playComplete();

    // Hamgiin chuhal: trip-iig 'completed' bolgoh (rider home ruu butsana).
    // Totals settle amжилтгүй boloвч энэ нь дэлгэц шилжихийг ХААХ ёсгүй.
    try {
      if (riderId.isEmpty || driverId.isEmpty) {
        await tripApi.updateTripStatus(tripId: tripId, status: 'completed');
      } else {
        await tripApi.completeTripAndSettleTotals(tripId: tripId, riderId: riderId, driverId: driverId, fare: fare);
      }
    } catch (e) {
      // Settle batch amжилтгүй bol ядаж status-iig completed bolgoхыг oroldno,
      // ингэснээр rider-iin дэлгэц гацахгүй.
      try {
        await tripApi.updateTripStatus(tripId: tripId, status: 'completed');
      } catch (_) {}
      Get.snackbar('Анхаар', 'Аялал дууссан (орлого шинэчлэхэд алдаа гарлаа): $e');
    }
    ...
    // Дэлгэц ҮРГЭЛЖ home ruu shiljine - aль ч aldaa энэ navigation-iig haahgui.
    _resetActiveTrip();
    Get.offAllNamed(AppRoutes.home);
  }
